-- Los cargos RELACIONADOS entran también en el conjunto de resultados, no solo en la puntuación.
--
-- Bug: `p_related_ids` se usaba en el `case` que puntúa (500 + peso*100) pero NO estaba en el
-- WHERE del `base`. Consecuencia: una oferta cuyo cargo es un relacionado solo aparecía si su
-- TEXTO contenía además el término buscado. En la práctica el tramo de "relacionados" no existía:
-- verificado con q="cocinero", tras las 592 ofertas de cargo exacto venían directamente las de
-- coincidencia por texto (posición 593), y ofertas de `sommelier`, `pastry chef` o `Jefe Partida`
-- —relacionados con peso alto y con ofertas activas— no salían nunca.
--
-- Orden resultante (el que se quiere): cargo EXACTO (1000) → RELACIONADOS de mayor a menor peso
-- (500 + peso*100, así que 0.9 → 590 y 0.5 → 550) → coincidencia por texto (100) → resto (0).
-- Las ya aplicadas van al final en todos los casos.
--
-- El corte por peso mínimo NO se hace aquí sino en resolveTitleContext (lib/job-board/
-- job-titles.ts): así el board y el asistente comparten el mismo criterio y se ajusta en un
-- solo sitio. La firma no cambia, solo el cuerpo.
