...
BEGIN
  ...
  VALUES (v_company_id, 'Junio 2026', '2026-06', v_company_name, 'monthly', 'in_review', 'USD')
  ...
  LOOP
    ...
    VALUES (
      v_company_id, v_employee.id, v_base, 'USD', 'monthly',
      '2026-01-01', 'transfer',
      CASE v_idx
        WHEN 1 THEN 'Banesco'
        WHEN 2 THEN 'Banesco'
      ...
      CASE v_idx
        ...
        ELSE NULL
      END,
      've', 'ISLR · retención mensual', v_company_name, v_total_emp
    )
    ...
    IF v_profile_id IS NOT NULL THEN
      ...
      VALUES
        (v_profile_id, 'Bono de alimentación (Cestaticket)', 'fixed',       400,                true,  0),
        (v_profile_id, 'Horas extra',                         'variable',    null,               true,  1),
        (v_profile_id, 'Bonificación anual',                  'conditional', round(v_base * 0.5, 2), true,  2),
        (v_profile_id, 'Bono por desempeño',                  'conditional', round(v_base * 0.15, 2), false, 3);
    END IF;
    ...
    IF v_line_id IS NOT NULL THEN
      ...
      VALUES
        (v_line_id, 'earning', 'Salario base',                   v_base,        '30 días', 0),
        (v_line_id, 'earning', 'Bono de alimentación (Cestaticket)', v_bono_alim, null,     1);
      IF v_hours_bonus > 0 THEN
        INSERT INTO pay_run_line_items (line_id, category, label, amount, quantity_label, order_index)
        VALUES (v_line_id, 'earning', 'Horas extra nocturnas', v_hours_bonus, '6 h', 2);
      END IF;

      -- Deducciones
      INSERT INTO pay_run_line_items (line_id, category, label, amount, order_index)
      VALUES
        (v_line_id, 'deduction', 'S.S.O. (4%)',                  v_sso_emp,  0),
        (v_line_id, 'deduction', 'R.P.E. paro forzoso (0,5%)',   v_rpe_emp,  1),
        (v_line_id, 'deduction', 'FAOV / LPH (1%)',              v_faov_emp, 2),
        (v_line_id, 'deduction', 'ISLR retención',               v_islr,     3);
      IF v_idx = 4 THEN  -- anticipo para el empleado con ajuste
        INSERT INTO pay_run_line_items (line_id, category, label, amount, order_index)
        VALUES (v_line_id, 'deduction', 'Anticipo de sueldo', 150, 4);
      END IF;

      -- Cargas patronales
      INSERT INTO pay_run_line_items (line_id, category, label, amount, order_index)
      VALUES
        (v_line_id, 'employer', 'S.S.O. patronal (9%)',            v_sso_pat,      0),
        (v_line_id, 'employer', 'R.P.E. patronal (1,7%)',          v_rpe_pat,      1),
        (v_line_id, 'employer', 'FAOV patronal (2%)',              v_faov_pat,     2),
        (v_line_id, 'employer', 'INCES (2%)',                      v_inces,        3),
        (v_line_id, 'employer', 'Provisión prestaciones sociales', v_prestaciones, 4);
    END IF;
  END LOOP;
  ...
  VALUES
    (v_company_id, 'Mayo 2026',     '2026-05', v_company_name, 'monthly', 'paid',     round(v_run_gross * 0.970, 2), round(v_run_net * 0.970, 2), round(v_run_employer * 0.970, 2), v_emp_count,                 'USD'),
    (v_company_id, 'Abril 2026',    '2026-04', v_company_name, 'monthly', 'paid',     round(v_run_gross * 0.890, 2), round(v_run_net * 0.890, 2), round(v_run_employer * 0.890, 2), GREATEST(v_emp_count - 1, 1), 'USD'),
    (v_company_id, 'Marzo 2026',    '2026-03', v_company_name, 'monthly', 'exported', round(v_run_gross * 0.870, 2), round(v_run_net * 0.870, 2), round(v_run_employer * 0.870, 2), GREATEST(v_emp_count - 1, 1), 'USD'),
    (v_company_id, 'Febrero 2026',  '2026-02', v_company_name, 'monthly', 'paid',     round(v_run_gross * 0.820, 2), round(v_run_net * 0.820, 2), round(v_run_employer * 0.820, 2), GREATEST(v_emp_count - 2, 1), 'USD'),
    (v_company_id, 'Enero 2026',    '2026-01', v_company_name, 'monthly', 'paid',     round(v_run_gross * 0.780, 2), round(v_run_net * 0.780, 2), round(v_run_employer * 0.780, 2), GREATEST(v_emp_count - 2, 1), 'USD');
