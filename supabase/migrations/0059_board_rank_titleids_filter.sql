-- Corrige el recall del RPC de ranking: la condición "sin texto → matchea todo" NO debe
-- dispararse cuando hay p_title_ids (hubs de cargo). Con titleIds sin q/tokens, el filtro base
-- debe ser SOLO ese cargo (antes el hub /empleos/camarero devolvía TODAS las ofertas).
create or replace function board_rank_jobs(
  ...
  p_applied uuid[], p_sort text, p_limit int, p_offset int, p_country text default null
