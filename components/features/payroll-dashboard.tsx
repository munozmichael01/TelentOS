  ...
  return (
    <div>
      ...
      >
        {currentRunId && (
          <button
            onClick={() => router.push(`/employer/payroll/runs/${currentRunId}`)}
          ...
          {alerts.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
              {alerts.map((a, i) => (
                <div
                  key={i}
                  onClick={() => currentRunId && router.push(`/employer/payroll/runs/${currentRunId}`)}
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.line}`, borderRadius: "16px", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", padding: "15px 20px", borderBottom: `1px solid ${T.line}` }}>
          ...
          <button
            onClick={() => router.push("/employer/payroll/runs")}
        ...
        {runs.map((r) => {
          ...
          return (
            <div
              key={r.id}
              onClick={() => router.push(`/employer/payroll/runs/${r.id}`)}
