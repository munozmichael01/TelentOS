    ...
    <div className="jb-fade">
      ...
      <section style={{ background: "var(--ink)", color: "#F4F0E8", borderRadius: 16, padding: 16, marginBottom: 14 }}>
        ...
        {!complete && pending.length > 0 && (
          <div style={{ display: "flex", gap: 6, marginTop: 13, flexWrap: "wrap" }}>{pending.map((c) => <Chip key={c.key} tone="neutral" onClick={onEdit}><PlusTiny />{t(`account.check.${c.key}`)}</Chip>)}</div>
        )}
      </section>
