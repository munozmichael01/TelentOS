      ...
      <div style={{ width: "100%", maxWidth: 400, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 9, marginBottom: 16 }}>
          {([["Google", "withGoogle"], ["LinkedIn", "withLinkedin"]] as const).map(([prov, key]) => (
            <button key={prov} onClick={() => setSoon(true)} className="jb-hard" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, fontFamily: ARCHIVO, fontWeight: 700, fontSize: 14, color: "var(--ink)", background: "var(--surface)", border: "2px solid var(--ink)", borderRadius: 11, padding: 12, boxShadow: "3px 3px 0 var(--ink)", cursor: "pointer" }}>
              {prov === "Google"
...
// Panel de marca (solo desktop, oculto en mobile por CSS): tinta + acentos lima, titular,
// 3 perks (Perfil IA · 1 toque · Alertas) y prueba social.
function AuthBrandPanel({ t }: { t: ReturnType<typeof useTranslations> }) {
  const perks = [
    { k: "perkProfile", icon: <path d="M12 2l2.4 7.2L22 12l-7.6 2.8L12 22l-2.4-7.2L2 12l7.6-2.8L12 2Z" /> },
    { k: "perkOneTap", icon: <path d="M13 2L4 14h7l-1 8 9-12h-7l1-8Z" strokeLinejoin="round" /> },
    { k: "perkAlerts", icon: <><path d="M6 9a6 6 0 1112 0c0 5 2 6 2 6H4s2-1 2-6Z" strokeLinejoin="round" /><path d="M10 19a2 2 0 004 0" /></> },
  ];
  return (
    <aside className="jb-auth-panel" style={{ background: "var(--ink)", color: "#F4F0E8", padding: "56px 48px", flexDirection: "column", justifyContent: "center", gap: 28 }}>
      <h2 style={{ fontFamily: ARCHIVO, fontWeight: 900, fontSize: 34, lineHeight: 1.05, letterSpacing: "-1.4px", maxWidth: 420, margin: 0 }}>{t("panelTitle")}</h2>
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {perks.map((p) => (
          <div key={p.k} style={{ display: "flex", alignItems: "center", gap: 13 }}>
            <span style={{ width: 40, height: 40, borderRadius: 12, background: "rgba(198,242,78,.14)", color: "var(--lime)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round">{p.icon}</svg>
            </span>
            <span style={{ fontSize: 15.5, fontWeight: 600, color: "#E8E4DB" }}>{t(p.k)}</span>
          </div>
        ))}
      </div>
      <div style={{ fontFamily: MONO, fontSize: 12, color: "#8C877E", marginTop: 8 }}>{t("socialProof")}</div>
    </aside>
  );
}
