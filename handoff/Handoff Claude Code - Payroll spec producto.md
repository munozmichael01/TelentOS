...
3. **Zona vetada para la pista de auditoría hasta que esta pista pase el paso 6:** `pay-run-detail.tsx`, `pay-profile.tsx`, `payroll-dashboard.tsx` y `app/(dashboard)/payroll/**` + `app/api/payroll/**`. La migración de design system de esas pantallas la hace la pista de auditoría *después*, sobre el código ya cableado.
   - **Excepción autorizada:** esta pista consolida las variantes canónicas en `components/ui/button.tsx` (primary/secondary/soft/ghost según el design brief) porque sus pantallas las necesitan — es el único archivo compartido que puede tocar. El barrido transversal de botones divergentes en el resto del producto es de la pista de auditoría, usando esas variantes.
