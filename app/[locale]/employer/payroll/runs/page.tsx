          ...
          {list.map((r) => {
            ...
            return (
              <HairlineRow key={r.id} align={["left", "left", "right", "right", "left", "right"]}>
                <Link href={`/employer/payroll/runs/${r.id}`} style={{ fontWeight: 700, color: "#1A1A17", textDecoration: "none" }}>{r.period_label}</Link>
                ...
                <Link href={`/employer/payroll/runs/${r.id}`} style={{ fontFamily: "'Space Mono',monospace", fontSize: "11px", fontWeight: 700, color: "#0E5C4A", textDecoration: "none" }}>
                  Ver →
                </Link>
              </HairlineRow>
            );
